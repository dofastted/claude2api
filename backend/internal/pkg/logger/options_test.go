...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/claude2api-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/claude2api-data", "logs", "claude2api.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "claude2api.log")
