...
func (f *GrokQuotaFetcher) BuildUsageInfo(account *Account) *UsageInfo {
	...
	if profile, ok := xai.SubscriptionProfileFor(account.GetCredential("subscription_tier")); ok {
		usage.SubscriptionTier = profile.Tier
		usage.SubscriptionTierRaw = profile.Tier
		if profile.ContextLimitTokens > 0 {
			limit := profile.ContextLimitTokens
			usage.GrokTokenQuota = &xai.QuotaWindow{Limit: &limit}
		}
	}
	...
	if snapshot.SubscriptionTier != "" {
		...
		if profile, ok := xai.SubscriptionProfileFor(snapshot.SubscriptionTier); ok {
			usage.SubscriptionTier = profile.Tier
		} else {
