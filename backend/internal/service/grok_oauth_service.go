...
func (s *GrokOAuthService) BuildAccountCredentials(tokenInfo *GrokTokenInfo) map[string]any {
	...
	if profile, ok := xai.SubscriptionProfileFor(tokenInfo.SubscriptionTier); ok {
		creds["subscription_tier"] = profile.Tier
		creds["context_limit_tokens"] = profile.ContextLimitTokens
		creds["usage_refresh_seconds"] = profile.UsageRefreshSeconds
		creds["usage_endpoint_available"] = profile.UsageEndpointAvailable
	} else if tokenInfo.SubscriptionTier != "" {
