...
func TestCodexEnvironmentProfilePoolBindsMixedSlots(t *testing.T) {
	...
	require.NoError(t, err)
	manager := NewEnvironmentProfileSlotLeaseManager()
	wants := map[EnvironmentClass]string{
		EnvironmentClassWindows: tlsfingerprint.ProfileNameCodexCLIWindows,
		EnvironmentClassLinux:   tlsfingerprint.ProfileNameCodexCLILinux,
		EnvironmentClassMacOS:   tlsfingerprint.ProfileNameCodexCLIMacOS,
		EnvironmentClassDesktop: tlsfingerprint.ProfileNameCodexDesktopDefault,
	}
	for _, env := range []EnvironmentClass{EnvironmentClassWindows, EnvironmentClassLinux, EnvironmentClassMacOS, EnvironmentClassDesktop} {
		...
		require.Equal(t, wants[env], profile.TLSProfile)
		lease.ReleaseFunc()
	}
...
func TestNewFrozenCodexEnvironmentProfilePoolUsesRegistryVersion(t *testing.T) {
	...
	registry.Swap(&clientidentity.Snapshots{
		...
		Codex: clientidentity.CodexSnapshot{
			Headers: map[string]string{
				"User-Agent": "codex_cli_rs/0.142.2 (Ubuntu 22.4.0; x86_64) xterm-256color",
				"originator": "codex_cli_rs",
			},
			VersionFields:  clientidentity.VersionFields{CLIVersion: "0.142.2"},
			TLSProfileName: tlsfingerprint.ProfileNameCodexCLIDefault,
		},
	})
	...
	for _, slot := range pool.Slots {
		require.Equal(t, "0.142.2", slot.Profile.Version)
		require.Contains(t, slot.Profile.UserAgent, "codex_cli_rs/0.142.2")
	}
}
