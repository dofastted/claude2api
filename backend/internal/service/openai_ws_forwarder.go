...
func resolveOpenAIWSSessionHeaders(c *gin.Context, promptCacheKey string) openAIWSSessionHeaderResolution {
	...
	if c != nil && c.Request != nil {
		if sessionID := strings.TrimSpace(c.Request.Header.Get(codexHeaderSessionID)); sessionID != "" {
		...
		if conversationID := strings.TrimSpace(c.Request.Header.Get(codexHeaderThreadID)); conversationID != "" {
			resolution.ConversationID = conversationID
			resolution.ConversationSource = "header_thread_id_hyphen"
			if resolution.SessionID == "" {
				resolution.SessionID = conversationID
				resolution.SessionSource = "header_thread_id_hyphen"
			}
...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	codexProfile *CodexEnvironmentProfile,
	...
	if account != nil && account.Type == AccountTypeOAuth {
		...
		if sessionResolution.SessionID != "" {
			isolated := isolateOpenAISessionID(apiKeyID, sessionResolution.SessionID)
			headers.Set("session_id", isolated)
			headers.Set(codexHeaderSessionID, isolated)
		}
		if sessionResolution.ConversationID != "" {
			isolated := isolateOpenAISessionID(apiKeyID, sessionResolution.ConversationID)
			headers.Set("conversation_id", isolated)
			headers.Set(codexHeaderThreadID, isolated)
			headers.Set(codexHeaderClientRequest, isolated)
		}
	} else {
		if sessionResolution.SessionID != "" {
			headers.Set("session_id", sessionResolution.SessionID)
			headers.Set(codexHeaderSessionID, sessionResolution.SessionID)
		}
		if sessionResolution.ConversationID != "" {
			headers.Set("conversation_id", sessionResolution.ConversationID)
			headers.Set(codexHeaderThreadID, sessionResolution.ConversationID)
			headers.Set(codexHeaderClientRequest, sessionResolution.ConversationID)
		}
	}
	...
	if account != nil && account.Type == AccountTypeOAuth {
		...
		headers.Set("originator", s.resolveOpenAIUpstreamOriginator(c, isCodexCLI))
	}
	...
	if s != nil && s.cfg != nil && s.cfg.Gateway.ForceCodexCLI {
		headers.Set("user-agent", s.codexCLIUserAgent())
	}
	...
	if codexProfile != nil {
		applyCodexEnvironmentProfile(&http.Request{Header: headers}, account, codexProfile, CodexProfileApplyOptions{
			APIKeyID:       getAPIKeyIDFromContext(c),
...
func setOpenAIWSClientMetadata(payload map[string]any, meta codexProfileRequestMetadata) bool {
	...
	if meta.InstallationID == "" && meta.SessionID == "" && meta.ThreadID == "" && meta.WindowID == "" && strings.TrimSpace(meta.TurnMetadata) == "" {
		return false
	}
	changed := false
	clientMetadata, ok := payload["client_metadata"].(map[string]any)
	if !ok {
		if existing, ok := payload["client_metadata"].(map[string]string); ok {
			clientMetadata = make(map[string]any, len(existing)+4)
			for k, v := range existing {
				clientMetadata[k] = v
			}
		} else if payload["client_metadata"] == nil {
	...
	setString := func(key, value string, override bool) {
		...
		if !override {
			if existing, ok := clientMetadata[key].(string); ok && strings.TrimSpace(existing) != "" {
				return
			}
		}
		if existing, ok := clientMetadata[key].(string); ok && strings.TrimSpace(existing) == value {
			return
		}
		clientMetadata[key] = value
		changed = true
	}
	setString(codexHeaderInstallationID, meta.InstallationID, false)
	setString("session_id", meta.SessionID, true)
	setString("thread_id", meta.ThreadID, true)
	setString(codexHeaderWindowID, meta.WindowID, true)
	setString(openAIWSTurnMetadataHeader, meta.TurnMetadata, true)
	return changed
}
...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	if codexProfile != nil {
		setOpenAIWSClientMetadata(payload, codexProfileRequestMetadataFor(account, codexProfile, CodexProfileApplyOptions{APIKeyID: getAPIKeyIDFromContext(c), PromptCacheKey: promptCacheKey}))
	} else {
		setOpenAIWSTurnMetadata(payload, turnMetadata)
	}
	wsHeaders, sessionResolution, headerErr := s.buildOpenAIWSHeaders(c, account, token, decision, isCodexCLI, turnState, turnMetadata, promptCacheKey, codexProfile)
	if headerErr != nil {
...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	wsHeaders, _, headerErr := s.buildOpenAIWSHeaders(c, account, token, wsDecision, isCodexCLI, turnState, strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader)), firstPayload.promptCacheKey, codexProfile)
	if headerErr != nil {
		return headerErr
	}
	...
	for {
		...
		if nextPayload.promptCacheKey != "" {
			...
			updatedHeaders, _, headerErr := s.buildOpenAIWSHeaders(c, account, token, wsDecision, isCodexCLI, turnState, strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader)), nextPayload.promptCacheKey, codexProfile)
			if headerErr != nil {
				return headerErr
			}
			baseAcquireReq.Headers = updatedHeaders
