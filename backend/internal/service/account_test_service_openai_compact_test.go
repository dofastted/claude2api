...
func TestAccountTestService_TestAccountConnection_OpenAICompactOAuthSuccessPersistsSupport(t *testing.T) {
	...
	require.Equal(t, upstream.lastReq.Header.Get("session-id"), gjson.Get(turnMetadata, "session_id").String())
	require.Equal(t, upstream.lastReq.Header.Get("thread-id"), gjson.Get(turnMetadata, "thread_id").String())
	require.Equal(t, upstream.lastReq.Header.Get("x-codex-window-id"), gjson.Get(turnMetadata, "window_id").String())
	require.Equal(t, "compaction", gjson.Get(turnMetadata, "request_kind").String())
