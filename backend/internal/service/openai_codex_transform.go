...
const (
	codexImageGenerationBridgeMarker = "<claude2api-codex-image-generation>"
	codexImageGenerationBridgeText   = codexImageGenerationBridgeMarker + "\nWhen the user asks for raster image generation or editing, use the OpenAI Responses native `image_generation` tool attached to this request. The local Codex client may not expose an `image_gen` namespace, but that does not mean image generation is unavailable. Do not ask the user to switch to CLI fallback solely because `image_gen` is absent.\n</claude2api-codex-image-generation>"
	codexSparkImageUnsupportedMarker = "<claude2api-codex-spark-image-unsupported>"
	codexSparkImageUnsupportedText   = codexSparkImageUnsupportedMarker + "\nThe current model is gpt-5.3-codex-spark, which does not support image generation, image editing, image input, the `image_generation` tool, or Codex `image_gen`/`$imagegen` workflows. If the user asks for image generation or image editing, clearly explain this model limitation and ask them to switch to a non-Spark Codex model such as gpt-5.3-codex or gpt-5.4. Do not claim that the local environment merely lacks image_gen tooling, and do not suggest CLI fallback as the primary fix while the model remains Spark.\n</claude2api-codex-spark-image-unsupported>"
)
...
//
// 加法式、幂等：installation_id 仅在缺失时由账号真实 device_id 注入；session/thread/window/turn
// 这些 host-owned metadata 始终覆盖为 profile 派生值，避免客户端污染新版 Codex 请求画像。
func applyCodexClientMetadata(reqBody map[string]any, account *Account, meta codexProfileRequestMetadata) bool {
	if reqBody == nil {
		return false
	}
	...
	if !ok {
		if existing, ok := reqBody["client_metadata"].(map[string]string); ok {
