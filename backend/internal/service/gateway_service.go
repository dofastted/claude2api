...
func (s *GatewayService) buildOAuthMetadataUserID(parsed *ParsedRequest, account *Account, fp *Fingerprint) string {
	...
	if userID == "" {
		userID = stableClaudeTelemetryUserID(account.ID, "legacy")
	}
...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		resp, err = s.httpUpstream.DoWithTLS(upstreamReq, proxyURL, account.ID, account.Concurrency, tlsProfileForRequest(upstreamReq, resolveClaudeCodeFamilyTLSProfile(account, s.tlsFPProfileService, true)))
		if err != nil {
			if resp != nil && resp.Body != nil {
				_ = resp.Body.Close()
			}
	...
	if account.IsOAuth() && s.identityService != nil {
		...
		if err != nil {
			...
			if claudeEnvironmentProfile != nil && claudeEnvironmentProfile.DeviceID != "" && isV2ClaudeEnvironmentProfile(claudeEnvironmentProfile) {
				...
				if newBody, err := s.identityService.RewriteUserIDWithSessionID(body, account.ID, accountUUID, claudeEnvironmentProfile.TelemetryUserID, fp.UserAgent, claudeEnvironmentProfile.TelemetrySessionID); err == nil && len(newBody) > 0 {
					body = newBody
				}
			} else if !enableMPT {
				accountUUID := account.GetExtraString("account_uuid")
...
func applyClaudeOAuthHeaderDefaults(req *http.Request, identityRegistry *clientidentity.Registry) {
	if req == nil {
		return
	}
...
func applyClaudeCodeMimicHeaders(req *http.Request, isStream bool, identityRegistry *clientidentity.Registry) {
	if req == nil {
		return
	}
	// Start with the standard defaults (fill missing).
	applyClaudeOAuthHeaderDefaults(req, identityRegistry)
	...
	if account.IsOAuth() && s.identityService != nil {
		...
		if err == nil {
			...
			if ctClaudeEnvironmentProfile != nil && ctClaudeEnvironmentProfile.DeviceID != "" && isV2ClaudeEnvironmentProfile(ctClaudeEnvironmentProfile) {
				accountUUID := account.GetExtraString("account_uuid")
				if newBody, err := s.identityService.RewriteUserIDWithSessionID(body, account.ID, accountUUID, ctClaudeEnvironmentProfile.TelemetryUserID, fp.UserAgent, ctClaudeEnvironmentProfile.TelemetrySessionID); err == nil && len(newBody) > 0 {
					body = newBody
				}
			} else if !ctEnableMPT {
				accountUUID := account.GetExtraString("account_uuid")
