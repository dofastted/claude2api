...
import (
	...
	"github.com/dofastted/claude2api/ent/authidentity"
	"github.com/dofastted/claude2api/ent/authidentitychannel"
	...
	"github.com/dofastted/claude2api/internal/pkg/httpclient"
	...
	"github.com/dofastted/claude2api/internal/util/httputil"
)
...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	if input.Platform == PlatformGrok && input.Type == AccountTypeOAuth {
		credentials = NormalizeGrokOAuthCredentials(credentials)
	}
	...
	if finalizeClaudeEnvironmentProfilePoolTelemetry(account) && account.Platform == PlatformAnthropic && account.Extra != nil {
		if pool, ok := account.Extra[claudeEnvironmentProfilePoolKey]; ok && s.accountRepo != nil {
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if len(input.Credentials) > 0 {
		...
		merged = SanitizeOAuthCredentialsForStorage(account.Platform, account.Type, merged)
		if account.Platform == PlatformGrok && account.Type == AccountTypeOAuth {
			merged = NormalizeGrokOAuthCredentials(merged)
		}
