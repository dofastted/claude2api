...
func TestEmailQueueTasksPreserveLocaleHints(t *testing.T) {
	...
	require.NoError(t, queue.EnqueueVerifyCode("user@example.com", "claude2api", "zh-CN"))
	require.NoError(t, queue.EnqueuePasswordReset("user@example.com", "claude2api", "https://example.com/reset", "en-US"))
...
func (s *notificationEmailTestSMTPServer) settings() map[string]string {
	...
	return map[string]string{
		...
		SettingKeySMTPFromName: "claude2api",
