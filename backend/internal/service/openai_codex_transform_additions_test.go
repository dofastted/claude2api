...
func TestApplyCodexClientMetadata(t *testing.T) {
	...
	body := map[string]any{}
	require.True(t, applyCodexClientMetadata(body, acc, codexProfileRequestMetadata{}))
	cm, ok := body["client_metadata"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "dev-xyz", cm["x-codex-installation-id"])
	// 幂等
	require.False(t, applyCodexClientMetadata(body, acc, codexProfileRequestMetadata{}))

	// OAuth 账号但无 device_id → 不写入（不伪造）
	body2 := map[string]any{}
	require.False(t, applyCodexClientMetadata(body2, &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth}, codexProfileRequestMetadata{}))
	...
	require.True(t, applyCodexClientMetadata(body3, acc, codexProfileRequestMetadata{SessionID: "profile-session", ThreadID: "profile-thread", WindowID: "profile-window", TurnMetadata: `{"session_id":"profile-session","thread_id":"profile-thread","window_id":"profile-window","request_kind":"turn"}`}))
	cm3, _ := body3["client_metadata"].(map[string]any)
	require.Equal(t, `{"session_id":"profile-session","thread_id":"profile-thread","window_id":"profile-window","request_kind":"turn"}`, cm3["x-codex-turn-metadata"])
	require.Equal(t, "dev-xyz", cm3["x-codex-installation-id"])
	require.Equal(t, "profile-session", cm3["session_id"])
	require.Equal(t, "profile-thread", cm3["thread_id"])
	require.Equal(t, "profile-window", cm3["x-codex-window-id"])
}
