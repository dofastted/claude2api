...
func (a *Account) GetClaudeEmail() string {
	for _, key := range []string{"email", "email_address", "claude_email", "anthropic_email"} {
...
func (a *Account) GetClaudeOrganizationID() string {
	for _, key := range []string{"organization_uuid", "org_uuid", "organization_id", "claude_organization_id", "anthropic_organization_id"} {
		if v := strings.TrimSpace(a.GetExtraString(key)); v != "" {
			return v
		}
		if v := strings.TrimSpace(a.GetCredential(key)); v != "" {
