...
const (
	...
	githubRepo     = "dofastted/claude2api"
...
func (s *UpdateService) PerformUpdate(ctx context.Context) error {
	...
	tempDir, err := os.MkdirTemp(exeDir, ".claude2api-update-*")
	if err != nil {
	...
	newBinaryPath := filepath.Join(tempDir, "claude2api")
...
func (s *UpdateService) extractBinary(archivePath, destPath string) error {
	...
	if strings.Contains(archivePath, ".tar") {
		...
		for {
			...
			if baseName == "claude2api" || baseName == "claude2api.exe" {
