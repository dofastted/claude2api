...
const (
	...
	codexHeaderClientRequest                          = "x-client-request-id"
)
...
type codexProfileRequestMetadata struct {
...
func (p *CodexEnvironmentProfile) Validate() error {
	...
	if p.InstallationID == "" {
		p.InstallationID = stableCodexInstallationID(p.SessionSeed)
	}
...
func newCodexEnvironmentProfile(family CodexClientFamily, source, ua, originator, version, tlsProfile string, headers map[string]string) (*CodexEnvironmentProfile, error) {
	...
	installationID := stableCodexInstallationID(sessionSeed)
	if err != nil {
		return nil, err
	}
...
func stableCodexInstallationID(seed string) string {
	trimmed := strings.TrimSpace(seed)
	if trimmed == "" {
		return ""
	}
	sum := sha256.Sum256([]byte("codex-installation:" + trimmed))
...
func codexTLSProfileForEnvironment(env EnvironmentClass, family CodexClientFamily) string {
	if family == CodexClientFamilyDesktop {
		return tlsfingerprint.ProfileNameCodexDesktopDefault
	}
	switch routeToSlot(env) {
	case EnvironmentClassWindows:
		return tlsfingerprint.ProfileNameCodexCLIWindows
	case EnvironmentClassMacOS:
		return tlsfingerprint.ProfileNameCodexCLIMacOS
	case EnvironmentClassLinux:
		return tlsfingerprint.ProfileNameCodexCLILinux
	default:
		return tlsfingerprint.ProfileNameCodexCLIDefault
	}
}
...
func codexProfileRequestMetadataFor(account *Account, profile *CodexEnvironmentProfile, opts CodexProfileApplyOptions) codexProfileRequestMetadata {
	if profile == nil {
		return codexProfileRequestMetadata{}
	}
	installationID := ""
	if account != nil {
	...
	if installationID == "" {
		installationID = strings.TrimSpace(profile.InstallationID)
	}
	if installationID == "" {
		installationID = stableCodexInstallationID(profile.SessionSeed)
	}
	...
	if meta.ThreadID != "" {
		meta.WindowID = generateUUIDFromSeed(fmt.Sprintf("codex-window::%d::%s", opts.APIKeyID, stableThread))
	}
	meta.TurnMetadata = buildCodexTurnMetadata(meta, opts)
	return meta
}
...
func applyCodexEnvironmentProfile(req *http.Request, account *Account, profile *CodexEnvironmentProfile, opts CodexProfileApplyOptions) {
	...
	if meta.InstallationID != "" {
		req.Header.Set(codexHeaderInstallationID, meta.InstallationID)
	}
	if meta.SessionID != "" {
		req.Header.Set("session_id", meta.SessionID)
		req.Header.Set("Session_Id", meta.SessionID)
		req.Header.Set(codexHeaderSessionID, meta.SessionID)
	}
	if meta.ThreadID != "" {
		req.Header.Set("conversation_id", meta.ThreadID)
		req.Header.Set("Conversation_Id", meta.ThreadID)
		req.Header.Set(codexHeaderThreadID, meta.ThreadID)
		req.Header.Set(codexHeaderClientRequest, meta.ThreadID)
	}
	if meta.WindowID != "" {
		req.Header.Set(codexHeaderWindowID, meta.WindowID)
	}
	if meta.TurnMetadata != "" {
		req.Header.Set(openAIWSTurnMetadataHeader, meta.TurnMetadata)
	}
}
