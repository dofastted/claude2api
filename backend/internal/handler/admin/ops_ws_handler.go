...
var upgrader = websocket.Upgrader{
	...
	// - The frontend passes ["claude2api-admin", "jwt.<token>"].
	// - We always select "claude2api-admin" so the token is never echoed back in the handshake response.
	Subprotocols: []string{"claude2api-admin"},
}
