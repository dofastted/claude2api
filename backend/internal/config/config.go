...
import (
	...
	"github.com/dofastted/claude2api/internal/pkg/proxyurl"
...
type GatewayConfig struct {
	...
	// GlobalProxyURL: 显式全局上游代理；为空时按账号代理/直连原逻辑执行。
	GlobalProxyURL string `mapstructure:"global_proxy_url"`
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	viper.AddConfigPath("/etc/claude2api")
...
func setDefaults() {
	...
	viper.SetDefault("log.service_name", "claude2api")
	...
	viper.SetDefault("database.dbname", "claude2api")
	...
	// Pricing - 从 model-price-repo 同步模型定价和上下文窗口数据；默认每天检查一次，避免频繁拉取但仍自动吸收新模型价格。
	...
	viper.SetDefault("dashboard_cache.key_prefix", "claude2api:")
	...
	viper.SetDefault("gateway.global_proxy_url", "")
...
func (c *Config) Validate() error {
	...
	if raw := strings.TrimSpace(c.Gateway.GlobalProxyURL); raw != "" {
		if _, _, err := proxyurl.Parse(raw); err != nil {
			return fmt.Errorf("gateway.global_proxy_url must be a valid proxy URL: %w", err)
		}
	}
	...
	if c.Gateway.UAAutoFetch.Interval < 0 {
		return fmt.Errorf("gateway.ua_auto_fetch.interval must be non-negative")
	}
...
func GetServerAddress() string {
	...
	v.AddConfigPath("/etc/claude2api")
