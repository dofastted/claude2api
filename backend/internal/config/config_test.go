...
func TestLoadGlobalProxyURLFromEnv(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("GATEWAY_GLOBAL_PROXY_URL", "http://127.0.0.1:8888")

	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, "http://127.0.0.1:8888", cfg.Gateway.GlobalProxyURL)
}

func TestValidateRejectsInvalidGlobalProxyURL(t *testing.T) {
	resetViperWithJWTSecret(t)
	cfg, err := Load()
	require.NoError(t, err)
	cfg.Gateway.GlobalProxyURL = "ftp://proxy.invalid:21"

	err = cfg.Validate()
	require.Error(t, err)
	require.Contains(t, err.Error(), "gateway.global_proxy_url")
}
...
func TestLoadDefaultDashboardCacheConfig(t *testing.T) {
	...
	if cfg.Dashboard.KeyPrefix != "claude2api:" {
		t.Fatalf("Dashboard.KeyPrefix = %q, want %q", cfg.Dashboard.KeyPrefix, "claude2api:")
	}
