...
func (s *HTTPUpstreamSuite) TestGlobalProxyOverridesAccountProxy() {
	s.cfg.Gateway = config.GatewayConfig{GlobalProxyURL: "http://global.proxy:8888"}
	svc := s.newService()

	entry, err := svc.getClientEntry("http://account.proxy:8080", 1, 1, service.HTTPUpstreamProfileDefault, false, false)
	require.NoError(s.T(), err)

	require.Equal(s.T(), "http://global.proxy:8888", entry.proxyKey)
}

func (s *HTTPUpstreamSuite) TestGlobalProxyOverridesTLSFingerprintProxy() {
	s.cfg.Gateway = config.GatewayConfig{GlobalProxyURL: "http://global.proxy:8888"}
	svc := s.newService()
	profile := tlsfingerprint.BuiltInProfileByName(tlsfingerprint.ProfileNameCodexCLIDefault)
	require.NotNil(s.T(), profile)

	entry, err := svc.getClientEntryWithTLS("http://account.proxy:8080", 1, 1, profile, service.HTTPUpstreamProfileOpenAI, false, false)
	require.NoError(s.T(), err)
