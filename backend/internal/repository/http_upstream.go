...
import (
	...
	"github.com/dofastted/claude2api/internal/pkg/proxyutil"
...
func (s *httpUpstreamService) effectiveProxyURL(proxyURL string) string {
	if s == nil || s.cfg == nil {
		return proxyURL
	}
	globalProxyURL := strings.TrimSpace(s.cfg.Gateway.GlobalProxyURL)
	if globalProxyURL == "" {
		return proxyURL
	}
...
func (s *httpUpstreamService) getClientEntryWithTLS(proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile, upstreamProfile service.HTTPUpstreamProfile, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
	...
	proxyURL = s.effectiveProxyURL(proxyURL)
