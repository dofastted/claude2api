export default {
  ...
  // Setup Wizard
  setup: {
    title: 'claude2api 安装向导',
    description: '配置您的 claude2api 实例',
    database: {
      title: '数据库配置',
  ...
  admin: {
    claudeOAuthPools: {
      ...
      description: '管理互迁策略、出口、shadow 指标与强制切换。环境胶囊由每个 OAuth 凭据自动生成并绑定，无需在此配置。',
      createPool: '新建池',
      empty: '尚未配置 Claude OAuth 池。',
      status: '状态',
      capsule: '胶囊',
      capsuleAuto: '自动（凭证级）',
      capsuleAutoNote: '每个 Claude OAuth 凭据会自动绑定 Windows / macOS / Linux 三个环境胶囊；入池时若缺失会自动补齐。',
      models: '允许模型',
      origins: '允许地址',
    ...
    backup: {
      ...
      r2Guide: {
        ...
        step1: {
          ...
          line2: '点击「创建存储桶」，输入名称（如 claude2api-backups），选择区域',
          line3: '点击创建完成'
        },
        step2: {
          title: '创建 API 令牌',
    ...
    accounts: {
      ...
      usageWindowsHint: '“5h / 7d”是上游账号（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对账号设定，并非 claude2api 配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置，无法在 claude2api 端解除该限制。',
      ...
      environmentProfile: {
        claudeTitle: 'Claude 环境 Profile 池',
        claudeDesc: '3 OS 槽位冻结(windows/macos/linux),每槽 1 个固定 device_id 与自洽身份;desktop 归并 windows;并发复用同槽。',
        codexTitle: 'Codex 环境 Profile 池',
        codexDesc: '3 OS 槽位冻结(windows/macos/linux),每槽固定身份;并发复用同槽;旧账号统一升级迁移。',
        singleEnvironment: '环境 Profile 池',
        singleEnvironmentDesc: '开启后按账号级 3 OS 槽位冻结式 Profile 出口;旧 Claude 账号不改动,Codex 旧账号迁移升级。',
        ...
        locked: '锁定 Profile',
        allowDesktopLearn: '允许学习 Desktop',
        allowDesktopLearnDesc: '默认关闭。开启后仅允许标准 Claude Desktop 写入固定 Profile。',
        allowOfficialClientLearn: '允许学习官方客户端',
        allowOfficialClientLearnDesc: '默认关闭。开启后仅允许标准 Codex CLI/Desktop/VSCode 写入固定 Profile。',
        editSlot: '编辑槽位',
        slotWindows: 'Windows 槽',
        slotMacos: 'macOS 槽',
        slotLinux: 'Linux 槽',
        deviceId: 'Device ID',
        clientId: 'Client ID',
        ...
        slotSaveSuccess: '槽位 Profile 已保存',
        slotSaveFailed: '保存槽位 Profile 失败',
        status: '当前 Profile',
        poolStatus: 'Profile 池:{count}/{capacity} 已绑定',
        slotTitle: '槽位 {slot} · {environment}',
        createPending: '新凭证落库时预生成 3 槽位;未绑定 pool 凭证首次请求懒生成。',
        noProfile: '尚未保存 Profile 或槽位。',
        reset: '重置 Profile',
        resetting: '重置中...',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 claude2api 实例的场景。',
    ...
    settings: {
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 claude2api 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 claude2api 用户登录',
  ...
  onboarding: {
    ...
    admin: {
      welcome: {
        title: '👋 欢迎使用 claude2api',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">claude2api 是一个强大的 AI 服务中转平台，让您轻松管理和分发 AI 服务。</p><p style="margin-bottom: 12px;"><b>🎯 核心功能：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>📦 <b>分组管理</b> - 创建不同的服务套餐（VIP、免费试用等）</li><li>🔗 <b>账号池</b> - 连接多个上游 AI 服务商账号</li><li>🔑 <b>密钥分发</b> - 为用户生成独立的 API Key</li><li>💰 <b>计费管理</b> - 灵活的费率和配额控制</li></ul><p style="color: #10b981; font-weight: 600;">接下来，我们将用 3 分钟带您完成首次配置 →</p></div>',
        nextBtn: '开始配置 🚀',
        prevBtn: '跳过'
      },
      groupManage: {
        title: '📦 第一步：分组管理',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;"><b>什么是分组？</b></p><p style="margin-bottom: 12px;">分组是 claude2api 的核心概念，它就像一个"服务套餐"：</p><ul style="margin-left: 20px; margin-bottom: 12px; font-size: 13px;"><li>🎯 每个分组可以包含多个上游账号</li><li>💰 每个分组有独立的计费倍率</li><li>👥 可以设置为公开或专属分组</li></ul><p style="margin-top: 12px; padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>💡 示例：</b>您可以创建"VIP专线"（高倍率）和"免费试用"（低倍率）两个分组</p><p style="margin-top: 16px; color: #10b981; font-weight: 600;">👉 点击左侧的"分组管理"开始</p></div>'
      },
      createGroup: {
        title: '➕ 创建新分组',
        description:
    ...
    user: {
      welcome: {
        title: '👋 欢迎使用 claude2api',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 claude2api AI 服务平台。</p><p style="margin-bottom: 12px;"><b>🎯 快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 创建 API 密钥</li><li>📋 复制密钥到您的应用</li><li>🚀 开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: '开始 🚀',
        prevBtn: '跳过'
      },
      keyManage: {
