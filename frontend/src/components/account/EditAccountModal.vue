<template>
  ...
  >
    ...
    >
      ...
      <!-- Grok OAuth model restriction / custom mapping -->
      <div
      ...
      >
        ...
        <div v-if="modelRestrictionMode === 'whitelist'">
          <ModelWhitelistSelector v-model="allowedModels" platform="grok" :account-id="account?.id" />
          <p class="text-xs text-gray-500 dark:text-gray-400">
            {{ t('admin.accounts.selectedModels', { count: allowedModels.length }) }}
            <span v-if="allowedModels.length === 0 && modelMappings.length === 0">{{
              t('admin.accounts.supportsAllModels')
      ...
      <!-- Anthropic OAuth / Setup Token model restriction -->
      <div
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    // Load model mappings for OAuth accounts with editable model restrictions
    if ((newAccount.platform === 'openai' || newAccount.platform === 'anthropic' || newAccount.platform === 'grok') && newAccount.credentials) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Anthropic OAuth/Setup Token: persist model restriction to credentials.
    ...
    // Grok OAuth: persist model restriction / custom mapping to credentials.
