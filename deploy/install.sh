#!/bin/bash
#
# claude2api Installation Script
# claude2api 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/dofastted/claude2api/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="dofastted/claude2api"
INSTALL_DIR="/opt/claude2api"
SERVICE_NAME="claude2api"
SERVICE_USER="claude2api"
CONFIG_DIR="/etc/claude2api"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="claude2api 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 claude2api 服务："
    ...
    ["not_installed"]="claude2api 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 claude2api。"
    ...
    ["uninstall_complete"]="claude2api 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 claude2api"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 claude2api"
    ...
    ["server_config_desc"]="配置 claude2api 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="claude2api Installation Script"
    ...
    # Completion
    ["install_complete"]="claude2api installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start claude2api service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading claude2api..."
    ...
    ["not_installed"]="claude2api is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove claude2api from your system."
    ...
    ["uninstall_complete"]="claude2api has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install claude2api"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove claude2api"
    ...
    ["server_config_desc"]="Configure claude2api server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/claude2api" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/claude2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="claude2api_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/claude2api" "$INSTALL_DIR/claude2api"
    chmod +x "$INSTALL_DIR/claude2api"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/claude2api"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/claude2api.service << EOF
[Unit]
Description=claude2api - AI API Gateway Platform
Documentation=https://github.com/dofastted/claude2api
...
User=claude2api
Group=claude2api
WorkingDirectory=/opt/claude2api
ExecStart=/opt/claude2api/claude2api
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=claude2api
...
start_service() {
    ...
    if systemctl start claude2api; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u claude2api -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable claude2api 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status claude2api"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u claude2api -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart claude2api"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop claude2api"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if claude2api is installed
    if [ ! -f "$INSTALL_DIR/claude2api" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/claude2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    ...
    cp "$INSTALL_DIR/claude2api" "$INSTALL_DIR/claude2api.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/claude2api.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/claude2api"
...
# Requires: claude2api must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/claude2api" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="claude2api.backup.${current_version}"
        else
            backup_name="claude2api.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/claude2api" "$INSTALL_DIR/$backup_name"
